...
class TasksModule {
    ...
    loadData() {
        const savedData = this.api.getModuleData('tasks');
        return {
    ...
    loadAIConfig() {
        try {
            const savedConfig = localStorage.getItem('tasksModule_aiConfig');
            const savedStats = localStorage.getItem('tasksModule_aiStats');
    ...
    saveAIConfigData() {
        try {
            localStorage.setItem('tasksModule_aiConfig', JSON.stringify(this.aiConfig));
            localStorage.setItem('tasksModule_aiStats', JSON.stringify(this.aiStats));
        } catch (error) {
    ...
    deleteScenario() {
        ...
        if (confirm(`¿Eliminar "${scenarioName}"? Esta acción no se puede deshacer.`)) {
            if (this.deleteScenarioById(this.editingScenario.id)) {
                this.hideNewScenarioModal();
    ...
    deleteScenarioConfirm(scenarioId) {
        ...
        if (confirm(`¿Eliminar "${scenario.name}"? Esta acción no se puede deshacer.`)) {
            if (this.deleteScenarioById(scenarioId)) {
    ...
    clearAllData() {
        ...
        localStorage.removeItem('tasksModule_aiConfig');
        localStorage.removeItem('tasksModule_aiStats');
        localStorage.removeItem('tasksModule_assistantMessages');
...
class TasksAssistant {
    constructor(tasksModule) {
        ...
        this.mensajes = JSON.parse(localStorage.getItem('tasksModule_assistantMessages')) || [];
    ...
    cleanHistory() {
        if (this.mensajes.length > this.tasksModule.aiConfig.historyLimit * 2) {
            ...
            localStorage.setItem('tasksModule_assistantMessages', JSON.stringify(this.mensajes));
        }
    }
    ...
    async consultarConFunciones(pregunta) {
        ...
        try {
            ...
            if (data.usage) {
                ...
                this.tasksModule.saveAIConfigData();
            }
